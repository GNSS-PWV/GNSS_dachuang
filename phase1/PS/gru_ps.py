...
def save_predictions_by_year_and_station(output_dir, model_preds, y_true, station_ids, years, doys, target_names):
    ...
    print(f"预测结果已按年份和站点保存至 {output_dir}")
...
def main():
    ...
    output_dir = r"/share/home/u23114/tj23114/packages/dachuang_pwv/PS/ps_result"
    ...
    try:
        ...
        # 按时间顺序和站点划分训练 / 验证 / 测试集
        print("按时间和站点划分训练 / 验证 / 测试集...", flush=True)
        X_train, X_val, X_test, y_train, y_val, y_test, doys_train, doys_val, doys_test, test_stations = time_based_split(
            X_seq, y_seq, times, years=years, station_ids=station_ids, doys=doys, test_station_ratio=0.1, random_state=42
        )
        ...
        # 按年份和站点保存预测结果
        print("按年份和站点保存预测结果...", flush=True)
        predictions_output_dir = os.path.join(output_dir, model_type, "predictions")
        ...
        scatter_path = os.path.join(
            output_dir, model_type, "scatter_gru_test.png"
        )
        plot_scatter(
            y_true,
            y_pred,
