...
class GNSSDownloader:
    ...
    def __init__(
        ...
        save_dir: str = "data/raw/validation/GNSS"
    ):
        """
        初始化GNSS下载器
    ...
    def download_euref_ztd(
        ...
        # 2019 ????? EPNE03??????? EPNFIN???????????
        candidate_names = [
            f"{station_upper}EPNFIN_{year}{doy}0000_01D_01H_TRO.TRO.gz",
            f"{station_upper}EPNE03_{year}{doy}0000_01D_01H_TRO.TRO.gz",
        ]
        ...
        if rename_to_short:
            ...
            save_path = save_dir / candidate_names[0]
        ...
        try:
            print(f"????EUREF??: {station_id.upper()}")
            if rename_to_short:
                print(f"????: {short_file_name}")

            for candidate_name in candidate_names:
                url = f"{self.DATA_SOURCES['euref']['url']}/{gps_week}/{candidate_name}"
                try:
                    response = requests.get(url, timeout=60)
                    response.raise_for_status()
                    if not rename_to_short:
                        save_path = save_dir / candidate_name
                    with open(save_path, 'wb') as f:
                        f.write(response.content)
            ...
            print(f"???? {station_id}: EPNFIN/EPNE03 ??????")
            return None
    ...
    def download_euref_analysis_center_files(
        ...
        def download_ac_file(ac: str) -> Optional[str]:
            """下载单个分析中心文件，兼容新旧 EUREF 文件命名。"""
            # 2019 年目录使用 EPNE03；较新的目录可能使用 EPNFIN。
            # 只尝试两个明确候选，不扩大到目录批量扫描。
            stems = (
                f"{ac}EPNFIN_{year}{doy}0000_01D_01H_TRO.TRO.gz",
                f"{ac}EPNE03_{year}{doy}0000_01D_01H_TRO.TRO.gz",
            )
            for file_name in stems:
