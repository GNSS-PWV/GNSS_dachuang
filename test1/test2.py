...
class LSTMAttentionModel(nn.Module):
    ...
    def __init__(self, input_size, hidden_size=64, num_layers=2, output_size=3, time_steps=24, dropout=0.3):
        ...
        self.attention = DynamicMultiHeadAttention(hidden_size, num_heads=4, dropout=0.2)
...
# ========== 模型训练（输出3个目标） ==========
def train_model(train_loader, val_loader, input_size, time_steps, model_type='lstm', output_size=3, epochs=100):
    if model_type == 'lstm':
...
def evaluate_models(X_test, y_test, model, test_loader, model_type="LSTM"):
    ...
    for i, name in enumerate(target_names):
        lstm_metrics[name] = {
            'mse': mean_squared_error(y_test[:, i], lstm_preds[:, i]),
            'rmse': np.sqrt(mean_squared_error(y_test[:, i], lstm_preds[:, i])),
            'mae': mean_absolute_error(y_test[:, i], lstm_preds[:, i]),
            'r2': r2_score(y_test[:, i], lstm_preds[:, i])
        }
    ...
    try:
        ...
        for model_type in ['lstm', 'cnn_lstm', 'cnn_bilstm', 'cnn_bilstm_attention']:  # 优先LSTM、CNN+LSTM、CNN+BiLSTM、CNN+BiLSTM+Attention
            ...
            metrics, lstm_preds = evaluate_models(
                X_test_scaled[:, -1, :], y_test_scaled,
                model, test_loader,
                model_type=display_name
            )

            # 反标准化并保存结果
            lstm_preds_original = y_scaler.inverse_transform(lstm_preds)
            save_predictions(os.path.join(output_dir, model_type), lstm_preds_original)

            # 在原始尺度下评估每个参数的精度
            target_names = ['TS', 'PS', 'WPS']
            if y_test_scaled.shape[1] > 3:
                target_names.append('Tm')
            
            y_true_original = y_scaler.inverse_transform(y_test_scaled)
