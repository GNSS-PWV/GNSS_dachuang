...
class ProfileTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        参数:
          ...
          use_global     : 是否使用全局特征(消融: 关掉则只用廓线)
          direct_pwv     : 是否直接预测 PWV 而非 Pi(消融: Pi参数化 vs 直接回归)
        """
        super().__init__()
        self.d_model = d_model
        self.use_global = use_global
        self.direct_pwv = direct_pwv
        ...
        # 融合 + 输出头: (CLS 输出 [+ 全局投影]) -> Pi 或 PWV
        head_in = d_model * 2 if use_global else d_model
    ...
    def forward(self, levels, heights, global_feat, attention_mask):
        ...
        # sigmoid 限制 Pi 范围
        pi = self.pi_min + torch.sigmoid(out) * self.sigmoid_scale
        return pi

    def predict_pwv(self, levels, heights, global_feat, attention_mask, zwd):
        """便捷方法: 返回 PWV (Pi*ZWD 或 direct_pwv 直接输出)."""
        out = self.forward(levels, heights, global_feat, attention_mask)
        if self.direct_pwv:
            return out
        return out * zwd