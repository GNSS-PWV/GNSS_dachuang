...
def plot_scatter(y_true, y_pred, save_path, title="Prediction vs True (WPS)"):
    """
...
def prepare_data(file_list, sample_ratio=1):
    ...
    for lag in [1, 2, 3, 6, 12]:
        data[f'WPS_lag{lag}'] = data.groupby(group_cols)['WPS'].shift(lag)

    # 为WPS添加额外的特征工程
    print("为WPS添加额外特征工程...")
    # WPS的水汽压差特征（使用已按组计算的滞后量）
    data['WPS_diff1'] = data['WPS'] - data['WPS_lag1']
    data['WPS_diff2'] = data['WPS_lag1'] - data['WPS_lag2']
    data['WPS_diff3'] = data['WPS_lag2'] - data['WPS_lag3']
    # WPS与TS的交互特征
    if 'TS' in data.columns:
        data['WPS_TS_ratio'] = data['WPS'] / (data['TS'] + 1e-8)
        data['WPS_TS_product'] = data['WPS'] * data['TS']
    # WPS与PS的交互特征
    if 'PS' in data.columns:
        data['WPS_PS_ratio'] = data['WPS'] / (data['PS'] + 1e-8)
        data['WPS_PS_product'] = data['WPS'] * data['PS']
    # WPS的移动平均特征（分组 rolling）
    ...
    # WPS的移动标准差特征
    for w in [3, 6]:
        data[f'WPS_std{w}'] = wps_group.rolling(window=w, min_periods=1).std().reset_index(level=group_cols,
                                                                                          drop=True)
...
def create_sequences(df, time_steps=24):
    ...
    feats = [
        ...
        'WPS_lag1', 'WPS_lag2', 'WPS_lag3', 'WPS_lag6', 'WPS_lag12', 'lat_sin', 'lat_cos', 'lon_sin', 'lon_cos',
        'elevation_normalized', 'distance_from_equator',
        # 新增WPS特征
        'WPS_diff1', 'WPS_diff2', 'WPS_diff3', 'WPS_TS_ratio', 'WPS_TS_product', 'WPS_PS_ratio', 'WPS_PS_product', 'WPS_ma3', 'WPS_ma6',
        'WPS_ma12', 'WPS_std3', 'WPS_std6',
        # 新增PS相关特征
...
def save_predictions_by_year_and_station(output_dir, model_preds, y_true, station_ids, years, doys, target_names):
    ...
    print(f"预测结果已按年份和站点保存至 {output_dir}", flush=True)
...
def evaluate_models(y_test, model, test_loader, model_type="GRU"):
    ...
    print(
        f"WPS: MSE: {model_metrics['mse']:.4f}, RMSE: {model_metrics['rmse']:.4f}, MAE: {model_metrics['mae']:.4f}, R2: {model_metrics['r2']:.4f}")
...
def main():
    ...
    output_dir = r"/share/home/u23114/tj23114/packages/dachuang_pwv/WPS/wps_result_a800"
    ...
    try:
        ...
        numeric_cols = ['YEAR', 'DOY', 'LAT', 'LON', 'ELV', 'TS', 'PS', 'WPS', 'Tm',
                        'hour', 'month', 'day', 'weekday', 'season', 'TS_lag1', 'PS_lag1', 'WPS_lag1', 'Tm_lag1',
                        'lat_sin', 'lat_cos', 'lon_sin', 'lon_cos',
                        'WPS_diff1', 'WPS_TS_ratio', 'WPS_PS_ratio', 'WPS_cumulative',
                        'WPS_ma3', 'WPS_ma6', 'WPS_ma12', 'WPS_std3', 'WPS_std6',
                        'PS_ma3', 'PS_ma6', 'PS_std3',
                        'TS_ma3', 'TS_ma6', 'TS_std3',
                        'elevation_normalized', 'distance_from_equator']
        ...
        target_names = ['WPS']
        print(f"序列形状：X={X_seq.shape}, y={y_seq.shape}（目标：WPS）", flush=True)
        ...
        # 绘制散点图
        scatter_path = os.path.join(output_dir, "gru", "scatter_gru_test.png")
        plot_scatter(y_test_original, model_preds_original, scatter_path, title="GRU Test Set (WPS)")

        # 保存预测结果
        save_dir = os.path.join(output_dir, "gru")
