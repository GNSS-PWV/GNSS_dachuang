...
def prepare_data(file_list, sample_ratio=1):
    ...
    for w in [3, 6]:
        data[f'PS_std{w}'] = ps_group.rolling(window=w, min_periods=1).std().reset_index(level=group_cols, drop=True)
...
def train_model(train_loader, val_loader, input_size, time_steps, model_type='gru', output_size=1, epochs=100, **kwargs):
...
def main():
    ...
    output_dir = r"/share/home/u23114/tj23114/packages/dachuang_pwv/PS/ps_result_2"
    ...
    try:
        ...
        for i, name in enumerate(target_names):
            ...
            print(f"  {name} 标准化完成：均值={scalers[name].mean_[0]:.2f}, 标准差={np.sqrt(scalers[name].var_[0]):.2f}", flush=True)

        print("\n使用最佳超参数训练最终模型...", flush=True)
        print("最佳超参数:")
        print("  learning_rate: 3.688684542235165e-05")
        print("  hidden_size: 132")
        print("  dropout: 0.10493794057341058")
        print("  weight_decay: 7.031096868737629e-06")
        print("  num_layers: 1")
        print("  batch_size: 32")
        ...
        best_params = {
            'learning_rate': 3.688684542235165e-05,
            'hidden_size': 132,
            'dropout': 0.10493794057341058,
            'weight_decay': 7.031096868737629e-06,
            'num_layers': 1
        }
        ...
        metrics, model_preds = evaluate_models(y_test_scaled, final_model, test_loader, model_type="GRU")
        ...
        y_test_original = inverse_transform_all(y_test_scaled, scalers, target_names)

        scatter_dir = os.path.join(output_dir, "gru")
        os.makedirs(scatter_dir, exist_ok=True)
        
        plot_scatter(y_test_original[:, 0], model_preds_original[:, 0], 
                    os.path.join(scatter_dir, "scatter_gru_test.png"), 
                    title="GRU Test Set (PS)")
        ...
        print(f"\n{'=' * 60}", flush=True)
        print(f"GRU 模型 - 原始尺度下PS的精度：", flush=True)
