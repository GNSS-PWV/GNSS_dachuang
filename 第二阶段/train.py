...
def pi_from_tm(tm):
    return 1e8 / (PHYS_RHO_W * PHYS_RV * (PHYS_K3 / tm + PHYS_K2P))
...
def train(args):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in train_loader:
            ...
            if getattr(args, 'phys_lambda', 0) > 0:
                # 物理一致性: Pi 应与"同一廓线的真值Tm对应线性公式"一致(软约束)
                tm = batch['tms'].to(device)
                pi_phys = pi_from_tm(tm)
                loss = loss + args.phys_lambda * F.smooth_l1_loss(pi_pred, pi_phys)
...
def main():
    ...
    parser.add_argument('--phys_lambda', type=float, default=0.0,
                        help='物理一致性正则权重(>0启用): L=L_pwv + lambda*|Pi-Pi(Tm)|')
